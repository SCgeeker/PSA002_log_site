...
info <- read.csv(lab_info_path) %>% subset(N!=0) %>% 
  left_join(
read.csv(rawdata_V_path) %>% group_by(PSA_ID) %>% summarise(collected_N = length(unique(subject_nr)), 
          begin_date = min(parse_date_time(datetime,orders = c('mdy HMS','b d HMS Y')) %>%                                                      format(format="%Y-%m-%d")), 
          lastest_date = max(parse_date_time(datetime,orders = c('mdy HMS','b d HMS Y','a b d HMS Y')) %>%                                                      format(format="%Y-%m-%d")))
)
